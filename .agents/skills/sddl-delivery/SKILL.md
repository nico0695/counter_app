...
Use `harness/sdd-lite/templates/artifacts/delivery-report.md` as the baseline shape.
...
`target-slug` follows the same charset and rules as a standalone review target (`harness/sdd-lite/skills/_shared/sddl-persistence-contract.md`): a branch target uses the branch name in kebab-case, a PR target uses `pr-{number}`, and any other target uses `commits-{first-short-sha}-{last-short-sha}`.
...
2. `harness/sdd-lite/templates/delivery/{commit,pr,ticket}.md` — the package default
