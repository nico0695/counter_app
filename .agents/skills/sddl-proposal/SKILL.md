...
When a gate needs the user, ask before writing the artifact. Use checkpoint type `missing_context` per `harness/sdd-lite/skills/_shared/sddl-user-interaction-contract.md`.
...
Use `harness/sdd-lite/templates/artifacts/proposal.md` as the baseline shape.
...
Before returning, apply the `phase_validation` checkpoint as defined in `harness/sdd-lite/skills/_shared/sddl-user-interaction-contract.md`. It is conditional: skip it when the user already indicated advancement, and always present it when scope, risk, or interpretation is ambiguous, or when the artifact carries open questions or risks above medium severity.
...
Return the common result structure from `harness/sdd-lite/skills/_shared/sddl-flow-contract.md`.
